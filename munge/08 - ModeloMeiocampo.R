info( logger, "CARTOLA_2017::modelo meio" )

info( logger, "CARTOLA_2017::filtra meio" )

meio <- dados_cartola_2017_por_rodada %>%
  filter( posicao == "mei" ) %>%
  mutate( fin = FF + FD + FT,
...
for( i in 1:(nrow(periodos) - janela) ){
  ...
  show( dadosTeste %>%
          ...
          ggtitle( paste0("meio_teste ", i) ) )
  
  meio_campo <- dadosTeste %>% 
    select( atleta_id, pontos_num_pred) %>% 
    mutate( rodada = tempoTeste ) %>%
    arrange( desc(pontos_num_pred) )
  
  save( meio_campo, file = paste0( "reports/meiocampo_rodada_", tempoTeste, ".RData") )
